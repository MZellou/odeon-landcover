...
def geo_projection_raster_guard(raster):
    ...
    try:
        if isinstance(raster, str):
            with rasterio.open(raster) as src:
                if src.crs == "" or src.crs is None:
                    raise OdeonError(
                        ErrorCodes.ERR_COORDINATE_REFERENCE_SYSTEM,
        ...
        raise OdeonError(
            ErrorCodes.ERR_IO,
            f"Odeon encountered an error during raster opening {raster}",
            stack_trace=rioe,
        )
...
def geo_projection_vector_guard(vector):
    ...
    if isinstance(vector, str):
        ...
        try:
            ...
            raise OdeonError(
                ErrorCodes.ERR_COORDINATE_REFERENCE_SYSTEM,
                f"the crs {src.crs} of vector {vector} has an encoding problem",
                stack_trace=error,
            )
    else:
        for v in vector:
    ...
    if isinstance(vector, str):
        with fiona.open(vector) as src:
            if src.driver not in FIONA_DRIVER_ACCEPTED:
                raise OdeonError(
                    ErrorCodes.ERR_DRIVER_COMPATIBILITY,
...
def file_exist(file):
    if os.path.isfile(file) is not True:
        raise OdeonError(
            ErrorCodes.ERR_FILE_NOT_EXIST, f"the file {file} doesn't exists"
        )
    ...
    for element in list_of_file:
        if isinstance(element, str):
            if os.path.isfile(element) is not True:
                raise OdeonError(
                    ErrorCodes.ERR_FILE_NOT_EXIST, f"the file {element} doesn't exists"
                )
        else:
...
def dirs_exist(list_of_dir):
    ...
    for dir_name in list_of_dir:
        if os.path.isdir(dir_name) is not True:
            raise OdeonError(
                ErrorCodes.ERR_DIR_NOT_EXIST, f"the dir {dir_name} doesn't exists"
            )
        ...
        if isinstance(raster, str):
            with rasterio.open(raster) as src:
                ...
                for band in list_of_band:
                    if band > bands_count:
                        raise OdeonError(
                            ErrorCodes.ERR_RASTER_BAND_NOT_EXIST,
                            f"the band {band} from raster {raster} " f"doesn't exists",
                        )
        else:
            for r in raster:
                with rasterio.open(r) as src:
                    ...
                    for band in list_of_band:
                        if band > bands_count:
                            raise OdeonError(
                                ErrorCodes.ERR_RASTER_BAND_NOT_EXIST,
                                f"the band {band} from raster {r} " f"doesn't exists",
                            )
    except rasterio.errors.RasterioError as rioe:
        raise OdeonError(
            ErrorCodes.ERR_IO,
...
def check_raster_bands(raster_band, proposed_bands):
    ...
    if isinstance(proposed_bands, list) and len(proposed_bands) >= 1:
        if not all((band in raster_band for band in proposed_bands)):
            LOGGER.error(
                "ERROR: the bands in the configuration file do not correspond\
